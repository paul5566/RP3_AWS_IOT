...
/*
 *	Reference Code from @champyen's project
 *		-https://github.com/champyen/cpusage
 *	Usage for RP3_Pi(32bit cpu)
 */
...
{
		char name[16];
		uint32_t tcks[NUM_TCK_TYPES];
} cpu_tck_t;

uint32_t idle_ticks(cpu_tck_t *stat)
{
		return stat->tcks[TCK_IDLE] + stat->tcks[TCK_IOWAIT];
}

uint32_t total_ticks(cpu_tck_t *stat)
{
		uint32_t total = 0;
...
void cpusage(cpu_tck_t *prev, cpu_tck_t *curr)
{
		...
		for(int i = 1; i <= nprocs; i++){
				uint32_t total = total_ticks(curr+i) - total_ticks(prev+i);
				uint32_t idle = idle_ticks(curr+i) - idle_ticks(prev+i);
				uint32_t active = total - idle;
